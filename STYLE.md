...
- **Background-failure strip (under the header, above the content):** the one banner the app ships, and only because the condition it reports *persists* — the repository stopped being readable, so the pane below is showing a snapshot that is no longer true. A modal would demand dismissal on every poll tick; a toast would vanish while the problem stayed. So: a single 12px line the full width of the content column, a `--status-yellow` warning triangle, plain sentence in `--text-primary`, git's own text after it in 11px mono `--text-muted` and ellipsized (it is data, and it is the part worth selecting), on a `color-mix` 12% wash of the same token with a bottom hairline. It has no dismiss control on purpose — it goes away when the repository comes back, and nothing else should make it go away. This is the *only* sanctioned banner: an error the user caused belongs in a modal, and everything informational belongs in a chip.
