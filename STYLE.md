...
- **The gutter and the prefix are chrome, not content**: both sit out of the text selection, so dragging across a diff puts the file's own lines on the clipboard and not the line numbers and `+`/`−` glyphs the viewer drew.
