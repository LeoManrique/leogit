...
- **A destructive text field locks behind Edit ▸ Done** instead of applying as you type: read-only on `--bg-secondary` with an **Edit** button below it, which becomes **Done**, and only Done writes. Scan paths are the case this exists for — a half-typed line is a different folder, and the list decides which repositories the app can see at all. Leaving mid-edit discards the draft; no confirmation anywhere in the cycle.
...
- Numeric settings (fetch interval, tab size, scan depth) are plain fields with tabular nums and **no stepper arrows** — a spinner is a second way to change a value that already has a keyboard, and under instant-apply each click of one is another write. macOS's `Stepper` is the exception the platform earns natively. Show the unit the user thinks in, not the one the wire carries: the fetch interval reads in seconds in both clients and is milliseconds only in `config.toml`.
