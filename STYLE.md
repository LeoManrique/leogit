...
- **What is not a banner.** A failure of an operation the user is waiting on — a transfer, a branch change, a commit — belongs in the modal, which is what "the thing you asked for did not happen" looks like. Anything merely informational belongs in a chip. The line between the modal and the strip is *is the user waiting on this*, not how bad it is.
