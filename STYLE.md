...
- **A dialog takes focus when it mounts** — `use:autofocus` on the modal container, or on its first field where it has one. `Escape` is handled *on* the dialog, so one that opens with focus still on the control that launched it cannot hear the key: it is raised somewhere else entirely and nothing dismisses. Suppress the container's focus ring; it is a listening post, not a control.
