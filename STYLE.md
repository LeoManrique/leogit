...
- **Transfer progress (fetch / push / pull / publish / clone):** progress lives *inside the control that started it*, GitHub-Desktop style — no toasts, banners, or floating progress windows. The sync button gets a full-height `--surface-hover` fill scaled with `transform: scaleX(fraction)` (`transform-origin: left`, 0.3 s ease-out so git's phase jumps glide instead of teleporting); button content layers above via `position: relative`, and the usual `:disabled` dimming is cancelled while the fill shows (the spinner already signals "busy" — graying the progress out would defeat it). **Where there is no percentage to show, the fill sweeps rather than sitting at zero** — a bar frozen at 0% reads as stuck, and fetch and publish never report one (nor does a push until git's first tick). Git's raw progress line renders in the header status area in 11px mono `--text-muted`, ellipsized. The Clone and Publish dialogs use a 4px rounded bar (`--border-active` fill on `--bg-secondary`), determinate for a clone and sweeping for a publish, above the same mono line.
