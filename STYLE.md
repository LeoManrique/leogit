...
- **Update chip (header, left of the action cluster):** informational, not an action the user owes us — so it's *tinted*, not solid: `--status-blue` text on a `color-mix` 12% wash of the same token with a 40% border (20% on hover), sitting a step below Pull/Push in the bar's hierarchy while still reading as new. Same 28px height and 6px radius as its neighbours, an up-arrow glyph, and no count badge. Never a banner, modal, or toast — an update is never urgent enough to take space from the repo.
