...
fn discover() -> Vec<ShellOption> {
    ...
    if let Some(path) = std::env::var("SHELL").ok().filter(|p| Path::new(p).is_file()) {
        let label = Path::new(&path)
            .file_name()
            .map_or_else(|| "Default shell".to_string(), |n| n.to_string_lossy().into_owned());
        shells.push(ShellOption {
            id: "default".into(),
    ...
    for (id, label, candidates) in [
        ("zsh", "zsh", ["/bin/zsh", "/usr/bin/zsh", "/usr/local/bin/zsh"]),
        ("bash", "bash", ["/bin/bash", "/usr/bin/bash", "/opt/homebrew/bin/bash"]),
        ("fish", "fish", ["/usr/local/bin/fish", "/usr/bin/fish", "/opt/homebrew/bin/fish"]),
...
fn fallback_shell() -> ShellOption {
    ShellOption {
        ...
        label: if cfg!(windows) { "Command Prompt" } else { "sh" }.into(),
