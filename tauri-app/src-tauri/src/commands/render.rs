...
fn render_line(
    ...
    for tok in tokens {
        ...
        render_slice(&chars, tok_start, tok_end, css_class(tok.class), overlay, &mut out);
        cursor = tok_end;
    }
    if cursor < chars.len() {
...
mod tests {
    ...
    fn plain_line_is_escaped_text_only() {
        assert_eq!(render_line("a < b && c > d", None, None), "a &lt; b &amp;&amp; c &gt; d");
    }

    #[test]
    fn tokens_emit_class_tagged_spans_and_plain_gaps() {
        let tokens = [token(0, 2, TokenClass::Keyword), token(3, 4, TokenClass::Plain)];
        assert_eq!(
    ...
    fn overlay_splits_spans_and_merges_classes() {
        let overlay = Some((IntraLineRange { start: 2, length: 3 }, "diff-intra-add"));
    ...
    fn indices_are_code_points_not_bytes() {
        ...
        let overlay = Some((IntraLineRange { start: 1, length: 2 }, "diff-intra-remove"));
        assert_eq!(
    ...
    fn malformed_token_bounds_are_clamped() {
        ...
        let tokens = [token(0, 3, TokenClass::Keyword), token(1, 99, TokenClass::String)];
        assert_eq!(
