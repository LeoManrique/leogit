...
// `shell_id` is owned because Tauri deserializes command arguments into owned
// values; a borrowed `Option<&str>` would tie the signature to the request's
...
mod tests {
    ...
    fn holds_truncated_tail_until_it_completes() {
        ...
        assert_eq!(decoder.push(&bytes[..1]), "", "must not emit a partial char");
    ...
    fn every_session_advertises_a_colour_capable_terminal() {
        for id in ["git-bash", "pwsh", "windows-powershell", "cmd", "default"] {
            ...
            assert!(env.contains(&("COLORTERM", "truecolor")), "{id} lacks COLORTERM");
        }
    }
    ...
    fn only_git_bash_opts_into_chere_invoking() {
        let has_chere =
            |id: &str| session_env(&shell_with_id(id)).iter().any(|(k, _)| *k == "CHERE_INVOKING");
