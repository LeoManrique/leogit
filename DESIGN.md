...
**Open from the terminal.** The installer adds a `leogit [dir]` shell command (analogous to `code .`): run it with no argument to open the current directory, or with a path to open that repo. If LeoGit is already running it focuses the existing window and switches to the repo rather than opening a second one; if the path isn't a git repo it just launches/focuses. See [TECHNICAL.md](TECHNICAL.md) → *Command-line repo opening* for how the shell function and the single-instance plumbing fit together.
