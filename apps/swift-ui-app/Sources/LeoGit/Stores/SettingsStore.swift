...
/// repo discovery, the terminal shell, and the AI knobs. Tauri-only fields
/// (theme, the diff toggles, …) cross the bridge untouched: `save()` loads a
/// fresh `Config`, overlays just the managed fields, and writes the whole
/// file back, so the other client's settings survive every save.
///
...
final class SettingsStore {
    ...
    /// The main window shares no store with the Settings scene; it picks the
    /// change up on its own — the auto-fetch loop re-reads the file every
    /// tick, and the rest is read on repo open.
