...
**`highlight_diff` must stay `#[tauri::command(async)]`.** A plain `#[tauri::command]` runs on the **main thread**. Tokenising whole blobs is ~20× more expensive than the old diff-only parse — the repo's largest file (`git.rs`, 3286 lines) measures **~52 ms release / ~284 ms debug** vs ~2 ms / ~14 ms diff-only — which is enough to beachball the cursor on every file switch. The old parse was cheap enough to hide that the command was on the UI thread at all. Same reasoning as the network commands in `git.rs` / `gh.rs`. Note the sibling diff commands (`parse_diff`, `get_diff`, `get_commit_diff`, `generate_patch`) are still sync; they're cheap enough today (a `git diff` subprocess plus a hunks-only parse) but sit on the same UI thread.
