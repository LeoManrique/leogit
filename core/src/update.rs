...
const INSTALL_COMMAND: &str = "curl -fsSL https://raw.githubusercontent.com/LeoManrique/leogit/main/scripts/install.sh | bash";
...
fn fake_update() -> Option<UpdateInfo> {
    let version = std::env::var("LEOGIT_FAKE_UPDATE")
        .ok()
        .filter(|v| !v.is_empty())?;
